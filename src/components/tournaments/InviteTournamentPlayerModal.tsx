...
const InviteTournamentPlayerModal = ({
  open, onOpenChange, tournamentId, tournamentName, existingPlayerIds,
}: Props) => {
  const { user } = useAuth();
  ...
  const handleInvite = async (targetUserId: string) => {
    ...
    await supabase.rpc("create_notification_for_user", {
      ...
      _type: "tournament_invite",
      _title: "Tournament Invitation",
      _body: `You've been invited to join "${tournamentName}"`,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md max-h-[80vh] flex flex-col p-0">
        <DialogHeader className="p-5 pb-0">
          ...
          <p className="text-xs text-muted-foreground mt-1">{tournamentName}</p>
        </DialogHeader>
...
export default InviteTournamentPlayerModal;
