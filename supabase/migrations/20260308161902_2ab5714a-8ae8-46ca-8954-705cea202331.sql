...
CREATE TABLE public.profiles (
  ...
  location TEXT,
  preferred_club TEXT,
  padel_level NUMERIC(2,1) CHECK (padel_level >= 0.5 AND padel_level <= 7.0),
  dominant_hand TEXT CHECK (dominant_hand IN ('left', 'right')),
  preferred_side TEXT CHECK (preferred_side IN ('left', 'right', 'both')),
  bio TEXT,
  padel_park_points INTEGER NOT NULL DEFAULT 0,
  reward_points INTEGER NOT NULL DEFAULT 0,
  total_matches INTEGER NOT NULL DEFAULT 0,
  wins INTEGER NOT NULL DEFAULT 0,
  losses INTEGER NOT NULL DEFAULT 0,
  reliability_score NUMERIC(4,1) NOT NULL DEFAULT 100.0 CHECK (reliability_score >= 0 AND reliability_score <= 100),
  matches_attended INTEGER NOT NULL DEFAULT 0,
