...
serve(async (req) => {
  ...
  try {
    ...
    const totalPoints = (pkg.total_points ?? pkg.points) + (pkg.bonus_points || 0);
    const amountCents = Math.round(pkg.price_gbp * 100);
    ...
    const session = await stripe.checkout.sessions.create({
        ...
        pkg.stripe_price_id
          ...
          : {
              price_data: {
                ...
                product_data: {
                  name: pkg.name,
                  description: `${totalPoints} Padel Park Points`,
                },
      ...
      success_url: `${origin}/rewards?points_success=true`,
