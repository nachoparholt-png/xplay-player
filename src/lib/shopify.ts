...
/**
 * Formats a Shopify price object into a human-readable string, e.g. "£7.00".
 * Falls back to the ISO code if the symbol isn't mapped.
 */
...
/**
 * Returns true if the product is available for sale in Shopify.
 * Uses the product-level availableForSale flag (Shopify aggregates all variants).
 */
export function shopifyInStock(product: ShopifyProduct): boolean {
      ...
      edges {
        node {
          ...
          metafields(identifiers: [
            { namespace: "custom", key: "xplay_points_price" }
          ]) {
            namespace
            key
            value
      ...
      products(first: $first) {
        edges {
          node {
            ...
            metafields(identifiers: [
              { namespace: "custom", key: "xplay_points_value" },
              { namespace: "custom", key: "xplay_redemption_cost" }
            ]) {
              key
              value
            }
...
export function getMetafieldValue(metafields: (ShopifyMetafield | null)[] | undefined, key: string): string | null {
